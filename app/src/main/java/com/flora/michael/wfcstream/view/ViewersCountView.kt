...
class ViewersCountView @JvmOverloads constructor(
    ...
    private val attributes: TypedArray = context.obtainStyledAttributes(attrs, R.styleable.ViewersCountView)
    ...
    init{
        ...
        val viewersIcon = attributes.getResourceId(R.styleable.ViewersCountView_viewersIcon, -1)

        if(viewersIcon != -1){
            setIcon(viewersIcon)
        }

        viewersCount = attributes.getInteger(R.styleable.ViewersCountView_viewersCount, 0)

        val viewersTint = attributes.getColor(R.styleable.ViewersCountView_viewersTint, -1)
    ...
    fun setTint(@ColorInt rgbInteger: Int){
        iconImageView.setColorFilter(rgbInteger, android.graphics.PorterDuff.Mode.MULTIPLY)
